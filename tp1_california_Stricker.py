# -*- coding: utf-8 -*-
"""
Created on Mon Aug 29 19:45:49 2022
...
X_cal_train = scale(X_cal_train)
X_cal_test = scale(X_cal_test)
# %%
# -------------------
# Ejercicio 1
# %%
...
clasificador_LR.fit(X_cal_train, y_cal_train)
predict_LR = clasificador_LR.predict(X_cal_test)
LR_score = clasificador_LR.score(X_cal_test, y_cal_test)
RMSE_LR = math.sqrt(mean_squared_error(y_cal_test, predict_LR))
# %%
# ----
# %%
...
ridge_sinCV.fit(X_cal_train, y_cal_train)
ridge_sinCV_score = ridge_sinCV.score(X_cal_test, y_cal_test)
ridge_sinCV_pred = ridge_sinCV.predict(X_cal_test)
MSE_sinCV_RR = math.sqrt(mean_squared_error(y_cal_test, ridge_sinCV_pred))
RMSE_sinCV_RR = math.sqrt(mean_squared_error(y_cal_test, ridge_sinCV_pred))
# %%
# ----
# %%
...
lasso_sinCV.fit(X_cal_train, y_cal_train)
lasso_sinCV_score = lasso_sinCV.score(X_cal_test, y_cal_test)
lasso_sinCV_pred = lasso_sinCV.predict(X_cal_test)
RMSE_sinCV_LS = math.sqrt(mean_squared_error(y_cal_test, lasso_sinCV_pred))
# %%
# ----
# %%
...
elasticnet.fit(X_cal_train, y_cal_train)
enet_sinCV_pred = elasticnet.predict(X_cal_test)
enet_sinCV_score = elasticnet.score(X_cal_test, y_cal_test)
RMSE_sinCV_enet = math.sqrt(mean_squared_error(y_cal_test, enet_sinCV_pred))
# %%
# %%
...
print("Utilizando un alpha de 0.8, el score correspondiente a Ridge es",
      format(ridge_sinCV_score))
print("Utilizando un alpha de 0.8, el score correspondiente a LASSO es",
      format(lasso_sinCV_score))
print("Utilizando un alpha de 0.8 y un l1 de 0.5, el score correspondiente a ElasticNet es",
      format(enet_sinCV_score))
print("\n")
print("\n")
# %%
# %%
...
print("Utilizando un alpha de 0.8, el RMSE correspondiente a Ridge es",
      format(RMSE_sinCV_RR))
print("Utilizando un alpha de 0.8, el RMSE correspondiente a LASSO es",
      format(RMSE_sinCV_LS))
print("Utilizando un alpha de 0.8 y un l1 de 0.5, el RMSE correspondiente a ElasticNet es",
      format(RMSE_sinCV_enet))
print("\n")
print("\n")
# %%
# ----
...
ridgecv = RidgeCV(alphas=alphas_ridge, gcv_mode='auto',
                  scoring='neg_mean_squared_error', normalize=True, cv=5)
ridgecv.fit(X_cal_train, y_cal_train)
...
ridge_score = ridge.score(X_cal_test, y_cal_test)
ridge_pred = ridge.predict(X_cal_test)
RMSE_RR = math.sqrt(mean_squared_error(y_cal_test, ridge_pred))
...
lassocv.fit(X_cal_train, y_cal_train)
lasso_alpha = lassocv.alpha_
lassocv_pred = lassocv.predict(X_cal_test)
lasso = Lasso(alpha=lasso_alpha, max_iter=1000000, normalize=True)
lasso.fit(X_cal_train, y_cal_train)
lasso_pred = lasso.predict(X_cal_test)
lasso_score = lasso.score(X_cal_test, y_cal_test)
RMSE_LS = math.sqrt(mean_squared_error(y_cal_test, lasso_pred))
...
elasticnetcv = ElasticNetCV(
    alphas=elastic_alpha, l1_ratio=elastic_l1_ratio, normalize=True, max_iter=1000)
elasticnetcv.fit(X_cal_train, y_cal_train)
elasticnetcv_seleccion_alpha = elasticnetcv.alpha_
elasticnetcv_seleccion_l1_ratio = elasticnetcv.l1_ratio_
elasticnetcv_prediccion = elasticnetcv.predict(X_cal_test)
elasticnet = ElasticNet(alpha=elasticnetcv_seleccion_alpha,
                        l1_ratio=elasticnetcv_seleccion_alpha)

elasticnet.fit(X_cal_train, y_cal_train)
enet_pred = elasticnet.predict(X_cal_test)
enet_score = elasticnet.score(X_cal_test, y_cal_test)
RMSE_enet = math.sqrt(mean_squared_error(y_cal_test, enet_sinCV_pred))
# %%


# %%
print(" Elastic Net score antes de CV es ", enet_sinCV_score, "con valor de alpha 0.8 y l1_ratio 0.5", '\n', "Elastic Net score luego de CV es",
      enet_score, " y los valores Alpha", elasticnetcv_seleccion_alpha, " y ratio l1 son ", elasticnetcv_seleccion_l1_ratio)
...
print("Si se observa el score, el Ridge parecería tener un mejor desempeño que el resto.\n")
print("Observando los valores de RMSE el Ridge también obtiene el mejor valor, por lo que se refuerza el razonamiento de que éste es el modelo que mejor se adapta al presente dataset.", "\n", "Si también se tiene en cuenta el modelo de linear regresion,", "\n", "esta sería la mejor ya que tiene el mayor score y el menor RMSE entre todos las regresiones realizadas.","\n")
...
print(
    'Los coeficientes encontrados usando Regresion Lineal son [w1 w2 w3 w4 w5 w6 w7 w8] =', clasificador_LR.coef_, '\n')
print(
    'Los coeficientes encontrados usando Regresion Ridge son [w1 w2 w3 w4 w5 w6 w7 w8] =', ridgecv.coef_, '\n')
print(
    'Los coeficientes encontrados usando Regresion Lasso son [w1 w2 w3 w4 w5 w6 w7 w8] =', lasso.coef_, '\n')
print(
    'Los coeficientes encontrados usando Regresion ElasticNet son [w1 w2 w3 w4 w5 w6 w7 w8] =', elasticnet.coef_, '\n')
# %%
